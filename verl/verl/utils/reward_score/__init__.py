...
def check_is_graph_task(data_source):
    graph_tasks = ['Connected', 'Diameter', 'Distance', 'GED', 'MCP', 'MCS', 'MIS', 'MVC', 'Neighbor', 'TSP']
    data_source_prefix = data_source.split('_')[0]
    return data_source_prefix in graph_tasks
...
def default_compute_score(
    ...
    if data_source == "openai/gsm8k":
        ...
        print(f"real_data_source: {real_data_source}, reward_type: {reward_type}")
        from . import npg_muse_rl
        res = npg_muse_rl.compute_score(solution_str, ground_truth, extra_info, task_name=real_data_source, reward_type=reward_type)
    elif data_source in math_tasks:
        from . import math
