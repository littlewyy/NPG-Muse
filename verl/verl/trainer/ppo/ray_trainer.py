...
class RayPPOTrainer:
    ...
    def _load_checkpoint(self):
        ...
        # 如果是从checkpoint加载，重新计算step数量
        total_training_steps = len(self.train_dataloader) + self.global_steps

        # if self.config.trainer.total_training_steps is not None:
        #     total_training_steps = self.config.trainer.total_training_steps

        self.total_training_steps = total_training_steps
        print(f"Recalculating Total training steps: {self.total_training_steps}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 修改这段代码：无论save_freq是否为0，最后一个step都要保存checkpoint
