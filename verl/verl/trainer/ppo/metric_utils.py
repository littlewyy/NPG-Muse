...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    """
    # add monitor of repeat_score:
    # 先试试，不成功再sum(-1)
    # repeat_score = torch.tensor(batch.non_tensor_batch["repeat_score"])
    # repeat_score = torch.tensor(batch.non_tensor_batch["repeat_score"]).sum(-1)
    ...
    metrics = {
        # repeat score
        # "critic/repeat_score/mean": torch.mean(repeat_score).detach().item(),
        # "critic/repeat_score/max": torch.max(repeat_score).detach().item(),
        # "critic/repeat_score/min": torch.min(repeat_score).detach().item(),
        # score
